...
4. MCP endpoint: `https://<your-domain>/mcp` (streamable HTTP).

## 4. Connect ChatGPT

1. In ChatGPT connector settings, add your Railway MCP URL: `https://<app>.up.railway.app/mcp`.
2. Use **Refresh** after deploying a new server version.
3. Call `get_bridge_diagnostics` to verify `schema_hash` and `tool_names` match the deployment.
